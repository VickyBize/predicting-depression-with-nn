...
for i in range (0,9,1):
    ...
    for j in range (0,len(Xml_files),1):
        ...
        night_test.insert(x, data[2])
        working_hours_test.insert(x, data[3])
        evening_test.insert(x, data[4])
        x_t=x_t+1;
...
os.chdir('C:\\Users\\Vicky\\Desktop\\Courses\\Μηχανες Αναζητησης\\erisk collections_2017_2018\\erisk collections\\2017\\1train')
...
for i in range (0,len(datatrain),1):#
    ...
    df3 = pd.merge(df2, df1 , how='left', left_on=0, right_on=0)
...
with h5py.File('datatest.h5', 'w') as hf:
...
with h5py.File('datatest2.h5', 'w') as hf:
